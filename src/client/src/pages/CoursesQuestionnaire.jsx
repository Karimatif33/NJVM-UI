...
import Spinner from "../components/Spinner";
  const CoursesQuestionnaire = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
          setIsLoading(true);
          try {
              const response = await axios.get(`https://njmc.horus.edu.eg/api/hue/portal/v1/Qus-CheckInstractorsExs/${DBUser}`);
              // console.log('Fetched data:', response.data);
              setAnsweredInstructorsData(response.data);

              // Find the first unanswered instructor
              const firstUnansweredIndex = response.data.findIndex(instructor => !instructor.answered);
  ...
  useEffect(() => {
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get(`https://njmc.horus.edu.eg/api/hue/portal/v1/Qus-CheckSubjectsExs/${DBUser}`);
...
// console.log(answeredSubjectData, "AnsweredSubjectData")
  ...
  const checkIfSubjectAnswered = (subjectId) => {
    return answeredSubjectData.some(answered => answered.subjectid === subjectId);
  };
  ...
  if (subject) {
    ...
    const checkIfInstructorAnswered = (subjectId, instructorId) => {
      return answeredInstructorsData.some(
        (instructor) => instructor.subjectid === subjectId && instructor.instructorid === instructorId
      );
    };
    ...
    const handleRadioChangeInstructor = (questionId, value) => {
      ...
      const allAnswered = Object.values(groupedQuestions).every((group) =>
        group.questions.every((question) => selectedOptionsInstructor[question.id])
      );
      setSubmitEnabled(allAnswered);
    };
    ...
    const handleSubmit = () => {
      ...
      console.log(questionId, "e")
        // Check if optionValue is an object with id and text properties
    ...
    const qusId = Number(questionId)
      ...
      axios
        .post("https://njmc.horus.edu.eg/api/hue/portal/v1/SubmitQuestions", data)
        .then((response) => {
    ...
    const checkIfAllInstructorsAnswered = () => {
      const answered = instructors.every(instructor =>
        answeredInstructorsData.some(answered => answered.instructorid === instructor.id)
      );
            ...
            {view === "course" ? (
              <div>
                ...
                {Object.entries(groupedQuestions).map(
                  ([headId, group]) =>
                    group.type === "subject" && (
                      <div key={headId} className="mb-6 bg-gray-100 p-8 rounded-2xl w-full wid dark:text-gray-200 dark:bg-secondary-dark-bg ">
                              <h3 className="text-2xl font-semibold mb-2 text-right dark:text-gray-200 dark:bg-secondary-dark-bg ">
                              {group.name}
                            </h3>
                              <hr />
                              {group.questions.map((question) => (
    <div key={question.id} className="mb-4 mt-2 bg-gray-200 py-5 rounded-2xl flex justify-between align-middle px-5 dark:text-gray-200 dark:bg-[#4d525e4f]">
        ...
        {question.options.map((option) => (
          <div key={`${question.id}_${option.id}`}>
            <input
              ...
              checked={selectedOptions[question.id]?.id === option.id} // Check if the selected option matches the current option's id
            ...
            <label
              ...
              className={`px-4 py-2 rounded-lg cursor-pointer ${
                selectedOptions[question.id]?.id === option.id
      ...
      <p className="mb-5 text-center textpx dark:text-gray-200">{question.description}</p>
    </div>
  ))}
                        <div className="mt-4">
                          <label
                ...
                <div>
                      {instructors.map((instructor) => (
                          ...
                          >
                            ...
                            {instructor.answered ? `(Answered) - ${instructor.name} ` : instructor.name}
              ...
              {activeInstructorIndex !== null && (
                  <div>
                      {Object.entries(groupedQuestions).map(
                          ([headId, group]) =>
                              (group.type === "assistant" || group.type === "doctor") && (
                                  <div
                                      key={headId}
                                  ...
                                  >
                                                  ...
                                                  {question.options.map((option) => (
                                                      <div key={`${question.id}_${option.id}`}>
                                                          <input
                                                              ...
                                                              value={option.id}
                                                              checked={
                                                                  selectedOptionsInstructor[question.id]?.id === option.id
                                                              }
                                                              onChange={() =>
                                                                  handleRadioChangeInstructor(question.id, option)
                                                              }
                                                              className={`mr-2 ${devAutFill}`}
                                                          />
                                                          <label
                                      ...
                                      <div className="mt-4">
                                          ...
                                          <textarea
                                              ...
                                              onChange={(e) =>
                                                  handleCommentChangeInstructor(headId, e.target.value)
                                              }
                                              className="dark:text-gray-200 dark:bg-[#424349] w-full p-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-600 focus:ring-opacity-75"
                                              rows="3"
                                          />
                    ...
                    answeredInstructorsData.length === 0 || !isLoading && !error && <p>No data available.</p>
                  )}
                </div>
